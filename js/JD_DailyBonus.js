        ...
        $nobyda.post(loginUrl, function(error, response, data) {
            try {
                if (error) {
                  ...
                  setTimeout(function() {
                    if (data.match(/\"login\":true/)) {
                      if (log) console.log("京东金融-京豆登录成功 response: \n" + data)
                      $nobyda.post(JDBUrl, function(error, response, data) {
                        try {
                          if (error) {
                            ...
                            const c = JSON.parse(data)
                            if (log) console.log("京东金融-白条提额 response: \n" + data)
                            if (c.resultCode==0) {
                              ...
                              if (data.match(/(发放失败|70111)/)) {
                                ...
                                if (data.match(/(\"resultCode\":3|请先登录)/)) {
                                  merge.JDBaitiao.notify = "京东金融-白条提额失败: 失败, 原因: Cookie 失效‼️"
                                  merge.JDBaitiao.fail = 1
                                } else {
                      ...
                      if (log) console.log("京东金融-白条提额失败登录失败 response: \n" + data)
                      if (data.match(/\"login\":false/)) {
                        merge.JDBaitiao.notify = "京东金融-白条提额失败: 失败, 原因: Cookie 失效‼️"
                        merge.JDBaitiao.fail = 1
                      } else {
